...
class Sales extends CI_Controller {
    ...
    public function updateSales(){
        ...
        try{
            ...
            // check previous sales payment type
            $oldSales = $this->db->query("select payment_type from tbl_salesmaster where SaleMaster_SlNo = ? and SaleMaster_branchid = ?", [$salesId, $this->session->userdata('BRANCHid')])->row();

            if($oldSales->payment_type == 'bank' && $data->sales->payment_type == 'cash') {
                $this->db->where('sale_id', $salesId)->delete('tbl_bank_transactions');
            }
            ...
            foreach($currentSaleDetails as $product){
                ...
                $this->db->query("
                    ...
                    set ps_s_r_status = 'no',
                    ps_s_r_amount = 0,
                    ps_s_status = 'no'
                    where sales_details_id = ?
                    and ps_brunch_id = ?
                ", [$product->SaleDetails_SlNo, $this->session->userdata('BRANCHid')]);
            }
    ...
    public function addSalesReturn(){
        ...
        try{
            ...
            foreach($data->cart as $product){
                ...
                $returnDetailId = $this->db->insert_id();
    ...
    public function getSaleReturns() {
        ...
        if(isset($data->id) && $data->id != '') {
            ...
            $res['returnDetails'] = array_map(function($detail) {
                $detail->serial = $this->db->query("
                    select 
                        ps.*
                    from tbl_product_serial_numbers ps 
                    where ps.sale_return_id = ?
                ", $detail->SaleReturnDetails_SlNo)->result();
    ...
    public function  deleteSales(){
        ...
        try{
            ...
            $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity, SaleDetails_SlNo')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();
            ...
            foreach ($saleDetails as $detail){
                ...
                $this->db->where('sales_details_id', $detail->SaleDetails_SlNo)->where('ps_prod_id', $detail->Product_IDNo)->update('tbl_product_serial_numbers', $serial);
            }
            ...
            $checkBankPayment = $this->db->query("select * from tbl_bank_transactions where sale_id = ? and branch_id = ?", [$saleId, $this->session->userdata('BRANCHid')]);
            if($checkBankPayment->num_rows() > 0) {
                $this->db->where('sale_id', $saleId)->delete('tbl_bank_transactions');
            }
